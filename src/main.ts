...
async function run(): Promise<void> {
  try {
    ...
    if (jwtToken) {
      if (botToken || chatId) {
        core.warning('bot-token and chat-id are ignored when notification-token is presented');
      }
    } else if (!botToken || !chatId) {
      throw new Error('bot-token and chat-id are required when notification-token is not presented');
    }
